...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        getJid: {
            value(sender, groupId) {
                try {
                    if (!sender.endsWith("@lid") || !groupId.endsWith("@g.us")) return sender;
                    const metadata = (conn.chats[groupId] || {}).metadata || conn.groupMetadata(groupId).catch(_ => null)
    ...
    enumerable: true
        ...
        sendSticker: {
            async value(jid, path, quoted, exif) {
                ...
                exif = { packName: options.packname || global.stickpack, packPublish: options.packpublish || global.stickauth  }
                let sticker = await (await import('./exif.js')).writeExif({ mimetype: mime, data }, exif);
                return conn.sendMessage(jid, { sticker }, { quoted });
            },
        },
        sendContact: {
    /**
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                const raw = this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.id || this.participant || this.key.participant || this.chat || '')
                return this.conn.getJid(raw, this.chat)
            },
            enumerable: true
        },
        fromMe: {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
                    ...
                    sender: {
                        get() {
                            ...
                            return conns.getJid(raw, this.chat)
                        },
                        enumerable: true
                    },
                    fromMe: {
                    ...
                    mentionedJid: {
                        get() {
                            ...
                            return raw.map(jid => conns.getJid(jid, this.chat))
                        },
                        enumerable: true
                    },
                    name: {
        ...
        mentionedJid: {
            get() {
                ...
                return raw.map(jid => this.conn.getJid(jid, this.chat))
            },
            enumerable: true
        },
        name: {
